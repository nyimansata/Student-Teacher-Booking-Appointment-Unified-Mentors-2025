...
export function renderAdminTeacherList(teachers, container) {
  ...
  teachers.forEach((t) => {
    ...
    el.querySelector(".teacher-name").textContent = t.name;
    el.querySelector(".teacher-email").textContent = t.email || "";
    el.querySelector(".teacher-dept").textContent =
      `Dept: ${t.department || "N/A"}`;
    el.querySelector(".teacher-subject").textContent =
      `Subject: ${t.subject || "N/A"}`;

    // Display approval status
    const statusEl = el.querySelector(".teacher-status");
    if (statusEl) {
      statusEl.textContent = t.approved ? "✅ Approved" : "⏳ Pending Approval";
      statusEl.style.fontSize = "0.9rem";
      statusEl.style.fontWeight = "600";
      statusEl.style.color = t.approved ? "#10b981" : "#f59e0b";
    }
      ...
      if (t.approved) {
        ...
        btnApprove.addEventListener("click", async () => {
          try {
            ...
            await updateTeacher(t.id, { approved: true });
            t.approved = true;
            el.remove();
            document.dispatchEvent(
              new CustomEvent("teacher-approved", { detail: { id: t.id } }),
            );
          } catch (err) {
            console.error(err);
    ...
    if (btnEdit) {
      btnEdit.addEventListener("click", () => {
        const dept = prompt("Department", t.department || "");
        if (dept === null) return;
        const subject = prompt("Subject", t.subject || "");
        if (subject === null) return;

        updateTeacher(t.id, { department: dept, subject })
          .then(() => {
            t.department = dept;
            t.subject = subject;
            el.querySelector(".teacher-dept").textContent =
            ...
            el.querySelector(".teacher-subject").textContent =
              `Subject: ${subject || "N/A"}`;
            document.dispatchEvent(
              new CustomEvent("teacher-updated", { detail: { id: t.id } }),
            );
          })
          .catch((err) => {
            console.error(err);
